...
void solve()
{
    ...
    vl appear;
    vl countarr;
    appear.resize(n + m + 1, -1);
    countarr.resize(n + m + 1, 0);
    vl arr(n + 1);
    for (int i = 1; i <= n; ++i)
    {
        ci(arr[i]);
    ...
    for (int i = 1; i <= n + m; ++i)
    {
        if (appear[i] != -1)
        {
            countarr[i] += (m + 1 - appear[i]);
        }
    }
    ll ans = 0;
    for (int i = 1; i <= n + m; i++)
        ans += countarr[i] * (countarr[i] - 1ll) / 2ll + countarr[i] * (m - countarr[i] + 1ll);
    co(ans); co(nl);
    return;
}
int main()
