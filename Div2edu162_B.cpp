...
bool cmp(pair<ll, ll>& a, pair<ll, ll>& b)
{
    if (a.first != b.first)
        ...
        return a.second > b.second;//shoot those with more health first.
}
void solve()
{
    ...
    for (int i = 0; i < n; ++i)
    {
        res.push_back({dist[i], arr[i]});
    }
    sort(res.begin(), res.end(), cmp);
    bool ok = 1;
    ll curdist = 0, tothealth = 0;
    for (int i = 0; i < (int)res.size(); ++i)
    {
        curdist = res[i].first;
        tothealth += res[i].second;
        if (tothealth > curdist * k)
        {
            ok = false; break;
        }
    }
